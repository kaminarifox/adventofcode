...
$y = 0;
$step = 1;
foreach ($wire0 as $value) {
    preg_match("/^([URDL])(\d+)$/", $value, $matches);
    $direction = $matches[1];
...
for ($i = -10000; $i < 10000; $i++) {
    for ($j = -10000; $j < 10000; $j++) {
        $key = "$j,$i";
        if (isset($coords[0][$key]) && isset($coords[1][$key])) {
            $intersections[$key] = $coords[0][$key] + $coords[1][$key];
        }
    }
}
