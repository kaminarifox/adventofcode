const arr = `myhposlqgeauywfikztndcvrqr
...
let two = 0, three = 0;
let twoCounter = 0, threeCounter = 0;
let letters = [...new Set(arr.join().split(''))];

arr.forEach((item) => {
    letters.forEach((letter) => {
        
        const re = new RegExp(letter, 'g');
        const l = (item.match(re) || []).length;
        
        if (l === 2 && !twoCounter) {twoCounter++;}
        else if (l === 3 && !threeCounter) {threeCounter++;}
        else if (twoCounter && threeCounter) {return;}
    });
